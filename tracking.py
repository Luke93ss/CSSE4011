...
class Kalman:
    def __init__(self, x_init, cov_init, meas_err, proc_err):
        ...
        self.dt = 0.5
        self.A = numpy.array([(1, 0, self.dt, 0), (0, 1, 0, self.dt), (0, 0, 1, 0), (0, 0, 0, 1)])
        self.H = numpy.array([(1, 0, 0, 0), (0, 1, 0, 0)])
        self.x_hat =  x_init
        self.cov = cov_init
        self.Q_k = numpy.eye(self.ndim)*proc_err
    ...
    def update(self, obs, update_meas_err):

        # Update estimate
        self.error_x = obs - numpy.dot(self.H,self.x_hat_est)
        self.error_cov = numpy.dot(self.H,numpy.dot(self.cov_est,numpy.transpose(self.H))) + numpy.eye(len(self.H))*update_meas_err
...
if __name__ == "__main__":		
    ...
    for t in range(nsteps):
        k.predict()
        ultraIsAvailable = (r.random() <= 0.5)
        k.update(obs[t], meas_error/10)
        if (ultraIsAvailable):
            k.update(obs2[t], meas_error/100)
        else:
            obs2[t] = 0
        x_hat[:,t]=k.x_hat
    ...
    pylab.scatter(obs2[:,0],   obs2[:,1],   s=10,c='y',marker='+', label="more accurate loc")
